...
%
% Deskripsi:
% Kode ini digunakan untuk membandingkan nilai MSE (Mean Squared Error) yang
% dinyatakan dalam skala log10 untuk tiga metode filtering (EKF, UKF, dan PF)
% pada berbagai tingkat frekuensi sampling (100 Hz, 500 Hz, dan 1000 Hz).
%
% Setelah data simulasi dari tiga metode ini diload ke workspace (dalam bentuk
% variable struktur ekf_ts100, ekf_ts500, ekf_ts1000, ukf_ts100, ukf_ts500,
% ukf_ts1000, pf_ts100, pf_ts500, pf_ts1000), kode ini akan menghitung rata-rata
% log10(MSE) serta standar deviasi untuk masing-masing metode pada tiap
% frekuensi sampling, lalu mem-plot hasilnya dalam bentuk errorbar.
%
% Setiap plot menggambarkan hasil estimasi untuk empat state yang berbeda:
% x1 (Rotor Angle), x2 (Rotor Speed), x3 (Transient voltage q-axis),
% x4 (Transient voltage d-axis).
%
...
% ekf_ts100, ekf_ts500, ekf_ts1000
% ukf_ts100, ukf_ts500, ukf_ts1000
% pf_ts100, pf_ts500, pf_ts1000

% Nama sumbu-x untuk kategori scenario (sampling time)
scenario_names = 'Process Noise';
...
ekf_process = {ekf_process0001, ekf_process001, ekf_process01};
ukf_process = {ukf_process0001, ukf_process001, ukf_process01};
pf_process  = {pf_process0001,  pf_process001,  pf_process01};

% Menghitung mean log10(MSE) untuk tiap skenario dengan menggunakan fungsi calculateMean
% serta standard deviasi dengan calculateStd. cellfun memudahkan kita untuk mengaplikasikan
% fungsi tersebut ke setiap elemen cell array tanpa loop manual.
logMSE_EKF = cell2mat(cellfun(@(x) calculateMean(x), ekf_process, 'UniformOutput', false));
logMSE_UKF = cell2mat(cellfun(@(x) calculateMean(x), ukf_process, 'UniformOutput', false));
logMSE_PF  = cell2mat(cellfun(@(x) calculateMean(x), pf_process,  'UniformOutput', false));

error_EKF = cell2mat(cellfun(@(x) calculateStd(x), ekf_process, 'UniformOutput', false));
error_UKF = cell2mat(cellfun(@(x) calculateStd(x), ukf_process, 'UniformOutput', false));
error_PF  = cell2mat(cellfun(@(x) calculateStd(x), pf_process,  'UniformOutput', false));
...
% Mengatur ukuran figure (dalam satuan inci) agar cocok dengan format two-column paper
...
for i=1:length(titles)
    ...
    % Menghitung batas sumbu y agar dapat menampung nilai errorbar secara penuh
...
% Fungsi calculateMean:
% Input: object (struktur data berisi x_true dan xMean)
% Tujuan: Menghitung mean dari nilai log10(error) pada setiap state
...
% Fungsi calculateStd:
% Input: object (struktur data berisi x_true dan xMean)
% Tujuan: Menghitung standard deviasi dari nilai log10(error) pada setiap state
% Output: stdObject (standar deviasi log10(MSE) dalam vektor kolom)
