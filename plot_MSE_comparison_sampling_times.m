...
scenario_names = 'Sampling Time';

% Kategori yang akan digunakan pada sumbu-x
categories = {'100 Hz', '500 Hz', '1000 Hz'};
...
ekf_ts = {ekf_ts100, ekf_ts500, ekf_ts1000};
ukf_ts = {ukf_ts100, ukf_ts500, ukf_ts1000};
pf_ts  = {pf_ts100, pf_ts500, pf_ts1000};
...
logMSE_EKF = cell2mat(cellfun(@(x) calculateMean(x), ekf_ts, 'UniformOutput', false));
logMSE_UKF = cell2mat(cellfun(@(x) calculateMean(x), ukf_ts, 'UniformOutput', false));
logMSE_PF  = cell2mat(cellfun(@(x) calculateMean(x), pf_ts,  'UniformOutput', false));

error_EKF = cell2mat(cellfun(@(x) calculateStd(x), ekf_ts, 'UniformOutput', false));
error_UKF = cell2mat(cellfun(@(x) calculateStd(x), ukf_ts, 'UniformOutput', false));
error_PF  = cell2mat(cellfun(@(x) calculateStd(x), pf_ts,  'UniformOutput', false));
