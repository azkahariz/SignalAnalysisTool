...
scenario_names = 'Without Noise';

% Kategori yang akan digunakan pada sumbu-x
categories = {'Without Noise'};
...
ekf = loadMat('ekf_wonoise');
ukf = loadMat('ukf_wonoise');
pf  = loadMat('pf_wonoise');
...
%%
logMSE_EKF = calculateMean(ekf);
logMSE_UKF = calculateMean(ukf);
logMSE_PF  = calculateMean(pf);

error_EKF  = calculateStd(ekf);
error_UKF  = calculateStd(ukf);
error_PF   = calculateStd(pf);
